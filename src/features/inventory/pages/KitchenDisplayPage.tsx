...
function modifierText(value: string | null | undefined, ar: boolean): string {
  if (!value) return '';
  try {
    const parsed = JSON.parse(value) as Array<{ option_name?: string; option_name_en?: string | null }>;
    if (!Array.isArray(parsed)) return value;
    return parsed
      .map((m) => ar ? (m.option_name || m.option_name_en || '') : (m.option_name_en || m.option_name || ''))
      .filter(Boolean)
      .join(' · ');
  } catch {
    return value;
    ...
    <DesignSurface testId="kitchen-display">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-ui-muted"><span className="inline-block w-2 h-2 rounded-full bg-ui-success mr-1 animate-pulse" />{items.length} {ar ? 'طلب' : 'orders'}</span>
        </div>
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map(item => (
            <div key={item.order_id} className={`rounded-2xl border bg-ui-surface p-4 shadow-ui-sm transition-all ${item.kitchen_status === 'cooking' ? 'border-ui-warning/40 ring-1 ring-ui-warning' : item.kitchen_status === 'ready' ? 'border-ui-success/40 ring-1 ring-ui-success' : 'border-ui-border'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-bold text-ui-text text-lg">#{item.order_number}</span>
                <div className="flex items-center gap-2">{statusBadge(item.kitchen_status, ar)}<span className={`text-sm ${elapsedColor(item.elapsed_seconds)}`}>{formatElapsed(item.elapsed_seconds)}</span></div>
              </div>
                  ...
                  return (
                    <li key={idx} className="rounded-lg bg-ui-page-alt px-2.5 py-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-ui-text font-bold">{it.product_name}</span>
                        <span className="text-ui-muted font-bold text-base">×{it.quantity}</span>
                      </div>
                      {mods && <div data-testid="kds-item-modifiers" className="mt-1 text-xs font-semibold text-ui-primary">{mods}</div>}
                    </li>
                  );
                })}
              </ul>
              ...
              <div className="flex gap-2 border-t border-ui-border pt-3">
                {item.kitchen_status === 'sent' && <button onClick={() => void handleKitchenStatus(item.order_id, 'cooking')} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-ui-warning-soft0 text-white py-3 px-4 text-sm font-bold hover:bg-ui-warning active:scale-95 transition-all min-h-[48px]"><ChefHat className="h-5 w-5" /> {ar ? 'بدء التحضير' : 'Start Cooking'}</button>}
                {item.kitchen_status === 'cooking' && <button onClick={() => void handleKitchenStatus(item.order_id, 'ready')} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-ui-success text-white py-3 px-4 text-sm font-bold hover:bg-ui-success active:scale-95 transition-all min-h-[48px]"><CheckCircle2 className="h-5 w-5" /> {ar ? 'جاهز للتقديم' : 'Mark Ready'}</button>}
                {item.kitchen_status === 'ready' && <button onClick={() => void handleKitchenStatus(item.order_id, 'served')} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-ui-info text-white py-3 px-4 text-sm font-bold hover:bg-ui-info active:scale-95 transition-all min-h-[48px]"><UtensilsCrossed className="h-5 w-5" /> {ar ? 'تم التقديم' : 'Served'}</button>}
              </div>
            </div>
          ))}
        </div>

        {!loading && !items.length && <div className="text-center py-16 text-ui-muted"><ChefHat className="h-12 w-12 mx-auto mb-3 opacity-30" /><div className="text-lg">{ar ? 'لا توجد طلبات في المطبخ' : 'No orders in kitchen'}</div></div>}
      </div>
    </DesignSurface>
  );
}
