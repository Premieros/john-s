              ...
              return (
                <article key={p.id} data-testid={`pos-product-card-${p.id}`} className={`group relative flex min-h-[176px] flex-col overflow-hidden rounded-2xl border bg-ui-surface text-start shadow-ui-sm transition ${blocked ? 'cursor-not-allowed border-ui-border opacity-55' : 'border-ui-border hover:-translate-y-0.5 hover:border-ui-primary hover:shadow-ui-md'}`}>
                  <button type="button" disabled={blocked} onClick={() => selectProduct(p)} className={`relative h-28 w-full overflow-hidden bg-ui-page-alt text-start ${blocked ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                    <ProductImage src={p.image_url} name={productLabel} category={categoryLabel} className="h-full w-full" imgClassName="h-full w-full object-cover transition duration-200 group-hover:scale-105" />
                    ...
                    <div className="mt-auto flex items-end justify-between gap-1 pt-2">
                      ...
                      {!blocked && (
                        <div className="flex shrink-0 items-center gap-1">
                          ...
                          <button type="button" aria-label={productLabel} title={isAr ? `إضافة ${productLabel}` : `Add ${productLabel}`} onClick={() => selectProduct(p)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-ui-primary text-ui-primary-fg shadow-ui-sm transition hover:bg-ui-primary/90 active:scale-95"><Plus className="h-4 w-4" /></button>
                        </div>
                      )}
                    </div>
                  </div>
