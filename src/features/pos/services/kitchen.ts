...
export async function reverseOrderKitchenConsumption(
  ...
  try {
    const res = await rpc<{ success: boolean; error?: string }>('reverse_order_kitchen_consumption', {
      p_order_id: orderId,
    ...
    console.warn('reverse_order_kitchen_consumption RPC error, falling back:', err);
  }
  ...
  try {
      ...
      if (c.raw_material_id) {
        ...
        try {
          await supabase.from('raw_material_movements').insert({
            ...
            notes: reason || 'Order canceled before preparation',
            created_at: timestamp,
          });
        } catch {
          // Continue
    ...
    return {
      success: false,
      error: err instanceof Error ? err.message : 'Failed to reverse kitchen consumption',
    };
  }
}
