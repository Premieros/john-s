-- Permission-First runtime reconciliation.
-- Capability comes only from roles.permissions. Role/membership labels only scope data.
...
BEGIN
  ...
  LOOP
    ...
    -- Historical owner admin pairs collapse to the sole implicit bypass.
    ...
    IF r.proname IN ('add_statement_line','match_bank_line','complete_bank_reconciliation','create_bank_reconciliation') THEN
      -- Historical definitions exist in both nested and combined forms.
      n := regexp_replace(n,
      ...
      n := regexp_replace(n,
        'IF[[:space:]]+get_user_role\(\)[[:space:]]+NOT[[:space:]]+IN[[:space:]]*\(''accountant''[[:space:]]*,[[:space:]]*''branch_manager''\)[[:space:]]+THEN',
        'IF NOT public.can_permission(''accounting.reconciliation.manage'') THEN', 'gi');
    END IF;
    ...
    -- Manager-label bypasses become an explicit override capability.
...
-- Explicit capability for assigning branch scope.
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM public.roles WHERE role=NEW.role AND is_active=true) THEN RAISE EXCEPTION 'UNKNOWN_ROLE'; END IF;
  IF auth.uid() IS NULL OR NOT EXISTS(SELECT 1 FROM public.users WHERE id=auth.uid() AND is_active=true) THEN RAISE EXCEPTION 'PERMISSION_DENIED'; END IF;
  IF TG_OP='INSERT' AND NOT public.can_permission('users.create') THEN RAISE EXCEPTION 'PERMISSION_DENIED:users.create';
  ELSIF TG_OP='UPDATE' AND NOT public.can_permission('users.manage') THEN RAISE EXCEPTION 'PERMISSION_DENIED:users.manage'; END IF;
  IF NEW.role='super_admin' AND NOT public.is_pos_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED: only Super Admin can assign super_admin'; END IF;
  IF TG_OP='UPDATE' AND OLD.role='super_admin' AND NOT public.is_pos_admin() THEN RAISE EXCEPTION 'PERMISSION_DENIED: only Super Admin can modify super_admin'; END IF;
  IF NEW.branch_id IS NOT NULL AND NOT public.user_may_access_branch(NEW.branch_id) THEN RAISE EXCEPTION 'PERMISSION_DENIED: branch outside caller scope'; END IF;
...
-- RLS: permission grants capability; membership and branch only define scope.
...
WITH CHECK (public.is_pos_admin() OR (public.can_permission('users.branches.manage') AND EXISTS(
...
USING (public.is_pos_admin() OR (public.can_permission('settings.manage') AND EXISTS(
  SELECT 1 FROM public.organization_members m WHERE m.organization_id=organizations.id AND m.user_id=auth.uid() AND m.is_active=true
)))
WITH CHECK (public.is_pos_admin() OR (public.can_permission('settings.manage') AND EXISTS(
  SELECT 1 FROM public.organization_members m WHERE m.organization_id=organizations.id AND m.user_id=auth.uid() AND m.is_active=true
)));
...
USING (public.is_pos_admin() OR (public.can_permission('users.branches.manage') AND EXISTS(
...
-- Fail closed on the resulting database contract.
DO $$
...
BEGIN
  ...
  WHERE ns.nspname='public' AND p.prokind='f'
    ...
    AND (
      position('''pos.sell''' in pg_get_functiondef(p.oid))>0 OR
      position('''pos.pay''' in pg_get_functiondef(p.oid))>0 OR
      position('''pos.split_order''' in pg_get_functiondef(p.oid))>0 OR
      position('''pos.transfer_order''' in pg_get_functiondef(p.oid))>0 OR
      position('''products.manage''' in pg_get_functiondef(p.oid))>0 OR
      position('''inventory.manage''' in pg_get_functiondef(p.oid))>0 OR
      position('''inventory.transfers''' in pg_get_functiondef(p.oid))>0 OR
      position('''inventory.transfers.approve''' in pg_get_functiondef(p.oid))>0 OR
      pg_get_functiondef(p.oid) ~ 'get_user_role\(\)[^;\n]*(owner|branch_manager|accountant|warehouse_manager|cashier)' OR
      ...
      pg_get_functiondef(p.oid) ~ 'membership_role[^;\n]*''owner'''
    );
