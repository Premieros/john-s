...
-- 1) Remove the owner label from operational users and organization membership.
...
-- 2) Delete legacy permission aliases from saved role permission arrays.
UPDATE public.roles
...
-- 3) The compatibility admin helper means Super Admin only.
...
-- 4) Canonical modifier administration.
...
BEGIN
  ...
  FROM (
    SELECT g.id, g.sort_order,
      jsonb_build_object(
        ...
        'options', COALESCE((
          SELECT jsonb_agg(
            jsonb_build_object(
              'id', o.id, 'name', o.name, 'name_en', o.name_en,
    ...
    WHERE g.product_id = p_product_id AND g.branch_id = v_branch_id AND g.is_active = true
  ) x;
...
-- 5) Rewrite only final runtime definitions. NOT IN is handled before IN to
-- avoid creating invalid `NOT =` syntax.
DO $$
DECLARE
  r record;
  d text;
...
-- 6) User and role management are capability-based; labels have no grants.
...
BEGIN
  ...
  -- Public/self registration is disabled. Only trusted registration bypasses this trigger.
...
-- 7) Organization owner membership is removed. `admin` is organization scope,
-- not an operational permission grant.
DROP POLICY IF EXISTS organization_members_insert ON public.organization_members;
CREATE POLICY organization_members_insert ON public.organization_members
FOR INSERT TO authenticated
WITH CHECK (
  public.is_pos_admin() OR EXISTS (
...
-- 8) Tenant registration no longer creates an owner role/membership.
DO $$
DECLARE r record; d text; n text;
BEGIN
  FOR r IN SELECT p.oid, pg_get_functiondef(p.oid) def
...
-- Demo operations use an explicit capability plus branch scope.
DO $$
DECLARE r record; d text; n text;
BEGIN
  FOR r IN SELECT p.oid, pg_get_functiondef(p.oid) def
           FROM pg_proc p JOIN pg_namespace ns ON ns.oid=p.pronamespace
           WHERE ns.nspname='public' AND p.prokind='f' AND p.proname IN ('seed_demo_data','delete_demo_data')
  LOOP
    d:=r.def;
    n:=replace(d,
...
-- 9) Fail closed. CI must not pass if drift survives this reconciliation.
DO $$
DECLARE
  v_count integer;
  v_objects text;
BEGIN
  ...
  FROM public.roles r CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(r.permissions,'[]'::jsonb)) p(permission)
  WHERE p.permission=ANY(ARRAY['pos.sell','pos.pay','pos.split_order','pos.transfer_order','products.manage','inventory.manage','inventory.transfers','inventory.transfers.approve','catalog.view','procurement.view','accounting.view','admin.view']);
  ...
  IF v_objects IS NOT NULL THEN
    RAISE EXCEPTION 'PERMISSION_FIRST_DRIFT: role-based or legacy authorization remains: %', v_objects;
  END IF;
END;
$$;
